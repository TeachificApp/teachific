 ...
 * CardioServ CME Activity Planning Form — org-scoped tRPC procedures
 *
 ...
 *   sendCmeFormToCardioServ(...)         — email PDF to CardioServ
 *   updateCardioServStatus(...)          — update approval status
...
/** Assert the org has CardioServ CME enabled (platform admins bypass) */
...
<div class="header">
  ...
  <p>${orgName} · CardioServ CME Processing</p>
</div>
...
<div class="footer">
  Generated by ${orgName} · CardioServ CME Processing · ${new Date().toLocaleDateString()}
</div>
</body>
</html>`;
...
export const cmeActivityFormRouter = router({
  ...
  sendCmeFormToCardioServ: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Build CC list: CardioServ staff + org's contact email
