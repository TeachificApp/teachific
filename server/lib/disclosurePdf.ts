 ...
 * Org-generic: orgName is passed in, no hardcoded brand names.
 */
...
export async function generateDisclosurePdf(data: DisclosurePdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(MED_GRAY).fontSize(8).font("Helvetica-Oblique")
      .text(`${data.orgName} is a CME joint provider with CardioServ, LLC.`, 50, y, { width: pageW });
    y += 14;
    ...
    // ─── Intro paragraphs ─────────────────────────────────────────────────────
    ensureSpace(60);
    const intro1 = "As a prospective planner or faculty member, we would like to ask for your help in protecting our learning environment from industry influence. Please complete the form below.";
    doc.fillColor(DARK).fontSize(8.5).font("Helvetica").text(intro1, 50, y, { width: pageW, lineGap: 1.5 });
    y += doc.heightOfString(intro1, { width: pageW, fontSize: 8.5 }) + 6;

    ensureSpace(55);
    const contactEmail = data.orgEmail || "admin@teachific.app";
    const intro2 = `The ACCME Standards for Integrity and Independence require that individuals who refuse to disclose relevant financial relationships be disqualified from involvement in the planning and implementation of accredited continuing education. Thank you for your diligence and cooperation. If you have questions, please contact: ${contactEmail}`;
    doc.fillColor(DARK).fontSize(8.5).font("Helvetica").text(intro2, 50, y, { width: pageW, lineGap: 1.5 });
    y += doc.heightOfString(intro2, { width: pageW, fontSize: 8.5 }) + 8;
    ...
    ensureSpace(45);
    const intro3 = "Please disclose all financial relationships that you have had in the past 24 months with ineligible companies (see definition below). For each financial relationship, enter the name of the ineligible company and the nature of the financial relationship(s). There is no minimum financial threshold; please disclose all financial relationships, regardless of the amount or perceived relevance to the educational activity.";
    doc.fillColor(DARK).fontSize(8.5).font("Helvetica").text(intro3, 50, y, { width: pageW, lineGap: 1.5 });
    y += doc.heightOfString(intro3, { width: pageW, fontSize: 8.5 }) + 10;
    ...
    for (let i = 0; i < range.count; i++) {
      ...
      doc.fillColor(MED_GRAY).fontSize(7).font("Helvetica")
        .text(
          `Financial Disclosure Form  ·  ${data.orgName}  ·  CME Joint Provider with CardioServ, LLC  ·  Submitted ${data.submittedAt.toLocaleDateString("en-US")}`,
