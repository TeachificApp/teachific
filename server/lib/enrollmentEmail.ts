...
async function resolveOrganizationBaseUrl(opts: {
  orgId?: number | null;
  orgSlug?: string | null;
  ...
  if (opts.orgSlug) {
    return getOrgBaseUrl(opts.orgSlug, opts.orgCustomDomain, opts.orgDomainVerificationStatus);
  }
  if (!opts.orgId) return null;
  const organization = await getOrgById(opts.orgId);
...
export async function sendEnrollmentEmail(opts: {
  ...
  const orgBase = await resolveOrganizationBaseUrl(opts);
  if (!orgBase) return false;
  const courseDestination = `${orgBase}/courses/${opts.courseSlug}`;
...
export async function sendDownloadAccessEmail(opts: {
  ...
  const orgBase = await resolveOrganizationBaseUrl(opts);
  if (!orgBase) return false;
  const filesDestination = `${orgBase}/downloads/${opts.productSlug}/files`;
...
export async function sendBundleAccessEmail(opts: {
  ...
  const orgBase = await resolveOrganizationBaseUrl(opts);
  if (!orgBase) return false;
  const bundleDestination = `${orgBase}/downloads/bundle/${opts.bundleSlug}`;
