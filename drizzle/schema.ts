...
export const quizzes = mysqlTable("quizzes", {
  id: int("id").autoincrement().primaryKey(),
  // The live QuizMaker table predates the canonical snake_case model. Keep its
  // established fields mapped explicitly while new metadata is added safely.
  orgId: int("orgId").notNull(),
  legacyPackageId: int("packageId"),
  legacyCreatedBy: int("createdBy"),
  title: varchar("title", { length: 500 }).notNull(),
  description: text("description"),
  coverImageUrl: varchar("cover_image_url", { length: 1024 }),
  instructions: text("instructions"),
  timeLimitSeconds: int("timeLimit"),
  legacyPassingScore: float("passingScore"),
  maxAttempts: int("maxAttempts"),
  passScorePercent: int("pass_score_percent").default(70).notNull(),
  legacyShuffleQuestions: boolean("shuffleQuestions"),
  legacyShuffleAnswers: boolean("shuffleAnswers"),
  legacyShowFeedbackImmediately: boolean("showFeedbackImmediately"),
  randomizeQuestions: boolean("randomize_questions").default(false).notNull(),
  randomizeAnswers: boolean("randomize_answers").default(false).notNull(),
