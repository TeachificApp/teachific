 ...
 *  - "bundle"      → pending bundle sales/refund module
 *  - "membership"  → pending membership sales/access module
 */
...
function BundleSalesTab({ bundleId }: { bundleId: number }) {
  return (
    <div className="rounded-lg border bg-white p-6 text-sm text-muted-foreground">
      Bundle sales, refunds, and access controls are not wired yet for bundle #{bundleId}. Bundle CRUD remains available.
    </div>
  );
}
...
function MembershipSalesTab({ brand }: { brand: "teachific" }) {
  return (
    <div className="rounded-lg border bg-white p-6 text-sm text-muted-foreground">
      Membership sales and access controls are not wired yet for {brand}. Membership CRUD remains available.
    </div>
  );
}
