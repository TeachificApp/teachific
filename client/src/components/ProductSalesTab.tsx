 ...
 *
 * Supports digital-download sales and membership access administration.
 * Bundle sales, refund, and manual-access operations remain unavailable until
 * their additive organization-scoped purchase model is implemented.
 */
...
// ─── Bundle Sales Tab ──────────────────────────────────────────────────────────
function BundleSalesTab({ bundleId: _bundleId }: { bundleId: number }) {
  return (
    <div className="rounded-lg border border-dashed border-[color:color-mix(in_srgb,var(--org-primary)_35%,transparent)] bg-[color:color-mix(in_srgb,var(--org-primary)_6%,transparent)] p-4 text-sm">
      <p className="font-medium text-foreground">Bundle sales and access controls are being upgraded.</p>
      <p className="mt-1 text-muted-foreground">Sales history, refunds, and manual access changes remain unavailable until bundle purchases are backed by the organization-scoped bundle data model. Existing learner access is unchanged.</p>
    </div>
  );
}
