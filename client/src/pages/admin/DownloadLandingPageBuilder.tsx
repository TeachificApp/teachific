...
export default function DownloadLandingPageBuilder() {
  ...
  const activeSourceBlocks = selectedSourceFunnelPageId ? sourceFunnelPageBlocks : selectedSourceCourseId ? sourceCourseBlocks : selectedSourceProductId ? sourceProductBlocks : sourceDownloadBlocks;
  ...
  return (
    ...
    <Dialog open={addMenuOpen} onOpenChange={open => { setAddMenuOpen(open); if (!open) setBlockSearch(""); }}>
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        ...
        {pickerTab === "from_pages" && (
          <div className="flex flex-1 overflow-hidden gap-3 min-h-0">
            <div className="w-52 shrink-0 flex flex-col gap-2 overflow-y-auto border-r border-gray-100 pr-2">
              ...
              <div className="border-t border-gray-100 pt-2">
                ...
                <select
                  ...
                  onChange={e => { setSelectedSourceDownloadId(e.target.value ? Number(e.target.value) : null); setSelectedSourceCourseId(null); setSelectedSourceProductId(null); setSelectedSourceFunnelId(null); setSelectedSourceFunnelPageId(null); setBlockSearch(""); }}
                >
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-w-0">
              {!selectedSourceDownloadId && !selectedSourceFunnelPageId && !selectedSourceCourseId && !selectedSourceProductId ? (
        ...
        {/* ── Templates tab ── */}
        {pickerTab === "templates" && (
          <DownloadTemplatesTab onInsertBlocks={(newBlocks) => { setBlocks(prev => [...prev, ...newBlocks]); if (newBlocks.length > 0) setSelectedId(newBlocks[0].id); toast.success(newBlocks.length === 1 ? "Template inserted!" : `${newBlocks.length} blocks inserted!`); setAddMenuOpen(false); }} />
        )}
      </DialogContent>
    </Dialog>
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button
            ...
            className="text-sm bg-[var(--org-primary)] hover:brightness-90 text-white px-4 py-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
// ─── Unified Templates Tab (page + block, shared pool) ───────────────────────
function DownloadTemplatesTab({ onInsertBlocks }: { onInsertBlocks: (blocks: Block[]) => void }) {
...
function getDefaultBlocks(title: string): Block[] {
  ...
  return [
    ...
    },
    {
      ...
      data: { headline: "Ready to Download?", subtext: "Get instant access to all files.", ctaText: "Buy Now", ctaColor: primary, ctaTextColor: "#ffffff", bgColor: "#ffffff", showPrice: true },
    },
  ];
}

// ─── Apply Template Modal ─────────────────────────────────────────────────────
function DownloadApplyTemplateModal({ onClose, onApply }: { onClose: () => void; onApply: (blocks: Block[]) => void }) {
  const [search, setSearch] = useState("");
  const utils = trpc.useUtils();
