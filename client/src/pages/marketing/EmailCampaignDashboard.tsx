    ...
    <div className="border rounded-xl p-4 bg-gray-50 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Display Name" className="text-sm" />
        </div>
        <div>
          <label className="text-xs text-gray-500 mb-1 block">From Email</label>
          <Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="sender@yourorganization.com" type="email" className="text-sm" />
        </div>
        <div>
...
export default function EmailCampaignDashboard() {
  ...
  if (!isAuthenticated || !user) return <DashboardLayout><div className="container py-12 text-center text-gray-500">Please log in.</div></DashboardLayout>;
  const isAdmin = user.role === "admin" || (user.appRoles ?? []).includes("platform_admin");
  if (!isAdmin) return <DashboardLayout><div className="container py-12 text-center"><Shield className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">Admin access required.</p></div></DashboardLayout>;
